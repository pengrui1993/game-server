...
//    static final byte BITS = 0b00____0000____00;
    ...
    private Card(boolean star,boolean joker,int v,int type) {
        if(star){
            value = (byte)(0b11111111&STAR_MASK);
        }else{
            if(joker){
                value = (byte)((0b11111111&JOKER_MASK)|((v<<VALUE_BIT_OFFSET)&VALUE_MASK));
            }else{
                value = (byte)(((v<<VALUE_BIT_OFFSET)&VALUE_MASK)|((type<<TYPE_BIT_OFFSET)&TYPE_MASK));
            }
        }
    }

    static int bit(int size,int shift){
        if(size<=0)return 0;
        int value=0;
        for(int i=0;i<size;i++)value|=(0x1<<i);
        value<<=shift;
        return value;
    }
    ...
    public boolean isClub(){
        return isNormalType()&&(TYPE_MASK&value)==TC;
    }
    public boolean isHearts(){
        return isNormalType()&&(TYPE_MASK&value)==TH;
    }
    public boolean isSpade(){
        return isNormalType()&&(TYPE_MASK&value)==TS;
    }
    public boolean isDiamond(){
        return isNormalType()&&(TYPE_MASK&value)==TD;
    }
    public boolean isStar(){
        return 0!=(STAR_MASK &value);
    }
    public boolean isJoker(){
        return 0!=(JOKER_MASK&value);
    }
    public boolean isRedJoker(){
        return isJoker()&&((VALUE_MASK&value)>>VALUE_BIT_OFFSET)==VRJ;
    }
    ...
    public static int typeCmp(Card l,Card r){
        ...
        if(l.isJoker())return l.value-r.value;
        return l.value&TYPE_MASK-r.value&TYPE_MASK;
    }
    ...
    public String toString() {
        if(isStar()){
            ...
            if(isJoker()){
                ...
                String t,v;
                final int val = (value&VALUE_MASK)>>VALUE_BIT_OFFSET;
                if(val<=10){
                    v =val+"";
                }else{
                    v = switch (val % 10) {
                ...
                final int type = (value&TYPE_MASK)>>TYPE_BIT_OFFSET;
                t = switch (type) {
